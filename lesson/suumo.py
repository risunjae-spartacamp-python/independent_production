...
def Recursive_PageNum(All_PageFullURLs, url):
    ...
    PageFullURLs = [
        urljoin("https://suumo.jp/", relative) for relative in PageURLs
    ]  # 相対パス -> 絶対パス
    # 1ページ目を先頭に格納
    ...
    if (
        len(nPages) == 0
        or len(nPages) == 1
        or (sorted(nPages)[-1] - sorted(nPages)[-2]) == 1
    ):
...
def ParseRoomDetail(EstateElem, url):
    # マンション名取得
    EstateName = EstateElem.find(
        "div", {"class": "cassetteitem_content-title"}
    ).get_text()
    # 住所取得
    EstateAddress = EstateElem.find(
        "li", {"class": "cassetteitem_detail-col1"}
    ).get_text()
    # 最寄り駅取得
    EstateLocationElem = EstateElem.find(
        "li", {"class": "cassetteitem_detail-col2"}
    ).find_all("div", {"class": "cassetteitem_detail-text"})
    EstateLocations = [
        EstateLocation.get_text() for EstateLocation in EstateLocationElem
    ]  # リストで取得
    EstateLocation = " --- ".join(EstateLocations)
    # 築年数と建物高さを取得
    EstateCol3Elem = EstateElem.find(
    ...
    for rooms in RoomtableElem.find_all("tbody"):
        ...
        RoomDetailURL = [
            link.get("href") for link in Roomlinks if link.get_text() == "詳細を見る"
        ][0]
...
def Parsedistrict(PageFullURLs, RoomDetails):
    for icount, url in enumerate(PageFullURLs):
        print(
            "    Room Detail Status: " + str(icount + 1) + "/" + str(len(PageFullURLs))
        )
        try:
            # データ取得
            result = requests.get(url)
...
body = soup.find("body")
RoomtableHeadElem = (
    body.find("div", {"class": "cassetteitem"}).find("thead").find_all("th")
)
...
HeaderNames = [
    temp.replace("\xa0", str(i)) for i, temp in enumerate(HeaderNames)
]  # 空白の場合行番号で置き換え
...
for iMcount, url in enumerate(BaseURLs):
    ...
    RoomDetails = Parsedistrict(
        All_PageFullURLs, RoomDetails
    )  # ページごとの物件情報取得 / Pageのループはこの中
